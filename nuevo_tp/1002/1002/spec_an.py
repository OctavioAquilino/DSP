...
class Plot_signal(GooCanvas.Canvas):
    def __init__(self, csig, ssig):
        super().__init__()
        self.set_bounds(0, 0, SIGNAL_WIDTH + SIGNAL_OFFS, SIGNAL_HEIGHT)
        cvroot = self.get_root_item()
        siglen = len(csig) - 1
        sig_scale = SIGNAL_WIDTH / siglen

        self.set_size_request(-1, SIGNAL_HEIGHT + 10)

        # Left plot (cosine based)
        ...
        points = GooCanvas.CanvasPoints.new(len(csig))
        for i, cs in enumerate(csig):
            points.set_point(i, i*sig_scale,
        ...
        points = GooCanvas.CanvasPoints.new(len(ssig))
        for i, ss in enumerate(ssig):
            points.set_point(i, SIGNAL_OFFS + i*sig_scale,
                                SIGNAL_HEIGHT//2 - ss * SIGNAL_SCALE)
        poly = GooCanvas.CanvasPolyline(
                    parent = cvroot,
                    points = points,
                    line_width = 2,
...
class Signal_editor(Gtk.Frame):
    def __init__(self, label, csig, ssig, tools):
        ...
        self.plotter = Plot_signal(self.csig, self.ssig)
    ...
    def on_edit_clicked(self, btn):
        ...
        # Columna extra con comandos para eliminar/agregar filas de datos
        # en las muestras
